...
\ Does this room have giant bats in it?
...
: input-unsigned-number ( -- uNumber flag )
    BEGIN
      CR ." > "
...
: arrow-path-backtrack-room ( index -- invalid-room )
    1 > IF
      ...
      \ Arrow can't go back where it came from
      OVER ARROW-PATH-ROOMS SWAP 2 - CHARS + C@
    ELSE
      \ Path has no more than one room
      \ Arrow can't stay in the player's room
      HUNTER C@
    THEN
  ;
...
\ Did the hunter get snatched by a giant bat?
...
\ Hunter went into a room with giant bats?
: entered-room-with-bats? ( -- flag )
    ...
    THEN
    \ Return TRUE if a giant bat moved the Hunter
  ; 
