...
: swap-random-in-tail ( i -- ) 
    DUP room-numbers + C@ SWAP DUP  \ ni i i
    DUP COUNT-ROOMS 1- SWAP -       \ ni i i max-index-within-tail
    1+ random                       \ ni i i index-within-tail
    + TUCK                          \ ni r i r
                                    \ r = index of room in tail
...
: prompt-command ( -- MOVE | SHOOT | ?WHERE )
    BEGIN
      CR ." What do you want to do "
         ." (m=move, s=shoot, w=where am I?)? "
      CR ." > "
      get-prompt-answer
      DUP MOVE = OVER SHOOT = OR OVER ?WHERE = OR
      DUP INVERT IF
        CR ." Huh? "
        SWAP DROP \ Lose invalid answer
      THEN
...
: prompt-play-again ( -- QUIT | SAME | NEW )
