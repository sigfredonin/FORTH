...
: prompt-command ( -- MOVE | SHOOT | ?WHERE )
    BEGIN
      ...
      CR ." > "
      get-prompt-answer ( --  answer )
      DUP  MOVE   =     ( answer -- answer =[MOVE]? )
      OVER SHOOT  = OR  ( answer =[MOVE]? -- answer =[MOVE|SHOOT]? )
      OVER ?WHERE = OR  ( answer =[MOVE|SHOOT]? -- answer valid )
      DUP INVERT IF
        CR ." Huh? "
        SWAP DROP \ Lose invalid answer
      THEN
...
: Hunt-the-Wumpus ( -- )
    ...
    BEGIN
      ...
      DUP QUIT =
    UNTIL
    DROP \ Lose the extra QUIT indicator
  ;
