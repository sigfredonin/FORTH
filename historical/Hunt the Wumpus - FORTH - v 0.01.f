...
: cave-room-address ( room-number - room-address)
  1- COUNT-NEIGHBORS COUNT-ROOMS * CAVE +
  ;
: cave-room-neighbors ( room-number - n_1 n_2 n_3 )
  cave-room-address DUP C@ SWAP 1+ DUP C@ SWAP 1+ C@
  ;
: ?cave-room-reachable ( to from - TRUE|FALSE )
  from cave-room-neighbors
    FALSE 1 4 ?DO     \ ( to n_1 n_2 n_3 flag - . . . )
      I PICK          \ 3..1, nth neighbor of from room
      5 PICK          \ to
      = IF
        DROP TRUE     \ ( to n_1 n_2 n_3 flag - to n_1 n_2 n_3 TRUE )
      THEN
    LOOP
