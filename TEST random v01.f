...
: swap-random-in-tail ( i -- ) \ Swap ith (i in 0..19) room with a room in i+1..20
    ...
    dup 20 swap - random      ( ni i -- in i t )    \ get random number, t in 0..19-i, index of a room number in the tail
    swap dup swap + tuck      ( in i t -- in r i r )      \ convert the index in the tail to one in the room numbers array  
...
: select-random-rooms ( n -- random-rooms ) \ select n <= 20 rooms at random
    ...
    THEN
      ...
      0 DO
        ...
        \ First i+1 room numbers are a random selection from 1..20, rest in order
...
: try-all-calc-t ( -- [ ni i t ] ) 20 0 do i try-calc-t CR .s clearstack loop ;
